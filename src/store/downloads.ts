...
interface DownloadsStore {
  ...
  startDownload: (video: VideoPost, profileUsername: string, profileUrl: string) => Promise<void>;
...
async function runDownload(
  ...
  const update = (u: Partial<DownloadItem>) => {
    ...
    storage
      ...
      .then((all) => {
        ...
        if (idx >= 0) storage.saveAllDownloads(all.map((i) => (i.id === item.id ? { ...i, ...merged } : i)));
      })
      .catch(() => {});
  };
  ...
  try {
    ...
    const localPath = await downloadVideo(downloadUrl, filename, destDir, (progress) => {
      update({ progress });
    });

    update({ status: 'done' as DownloadStatus, progress: 1, localPath });
    if (item.profileUsername && item.videoId) {
      useProfilesStore.getState().markVideoDownloaded(item.profileUsername, item.videoId);
    }
  } catch (err) {
    update({ status: 'failed' as DownloadStatus, error: err instanceof Error ? err.message : 'Download failed' });
  }
}
...
export const useDownloadsStore = create<DownloadsStore>((set, get) => ({
  ...
  retryDownload: async (id) => {
    ...
    if (!item) return;
    const reset: DownloadItem = { ...item, status: 'pending', error: undefined, progress: undefined };
